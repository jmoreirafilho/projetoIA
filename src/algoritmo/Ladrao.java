...
public class Ladrao extends ProgramaLadrao {
    ...
    public int acao() {
        ...
        // Carrega as ponderações (pensa)
        ponderacoes();

        // Pega a posição atual (antes de se mover)
        ultimaPosicaoAgente = sensor.getPosicao();

        // Utiliza visão e olfato para saber a viabilidade e o peso de ir pra cada direção
        ...
        // Ordena o peso de cada movimento
        Collections.sort(auxList);

        // acao = melhor movimento
    ...
    public void ponderacoes() {
        ...
        // Se tiver na mesma posição (parado)
        if (ultimaPosicaoAgente.equals(sensor.getPosicao())) {
            espacoVazio = 500;
            parede = -100;
        }

        if (procurandoBanco || perseguindo) {
            aliado = 100;
            banco = 200;
    ...
    public Double calcularFrente(int[][] matrizVisao, int[][] matrizCheiro) {
        ...
        boolean esquerdaNaoEstaVazia = matrizVisao[1][2] != 0;
        boolean naoTemNenhumPoupadorNaFrente = !(matrizVisao[2][1] >= 100 && matrizVisao[2][1] < 200);
        
        if (esquerdaNaoEstaVazia && naoTemNenhumPoupadorNaFrente) {
        	sum += obstaculo;
        }
        ...
        if (encontrouUmBanco) {
            distanciaParaBanco = calculaDistancia(pAtual[0] - 1, pAtual[1], pBank[0], pBank[1]);
            if (distanciaParaBanco > zonaSegura) {
            	sum += foraDaZona;
            }
        }
        return sum;
    ...
    public Double calcularAtras(int[][] matrizVisao, int[][] matrizCheiro) {
        ...
        boolean frenteNaoEstaVazia = matrizVisao[3][2] != 0;
        boolean naoTemNenhumPoupadorAtras = !(matrizVisao[2][1] >= 100 && matrizVisao[2][1] < 200);

        if (frenteNaoEstaVazia && naoTemNenhumPoupadorAtras)
            sum += obstaculo;
    ...
    public Double calcularEsquerda(int[][] matrizVisao, int[][] matrizCheiro) {
        ...
        if (encontrouUmBanco) {
            distanciaParaBanco = calculaDistancia(pAtual[0], pAtual[1] - 1, pBank[0], pBank[1]);
            if (distanciaParaBanco > zonaSegura) {
            	sum += foraDaZona;
            }
